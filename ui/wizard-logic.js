...
async function submitWizard() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('Failed to create ' + wizardState.profileType);
        }
        ...
        alert('Error creating ' + wizardState.profileType + '. Please try again.');
    }
}
...
function cancel() {
    if (confirm('Are you sure you want to cancel? Any unsaved progress will be lost.')) {
        localStorage.removeItem('wizardDraft');
        window.location.href = '/ui/index.html?token=' + token;
    }
}
